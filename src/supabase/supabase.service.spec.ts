...
describe('Supabase service client isolation', () => {
  ...
  it.each(['sb_publishable_test', jwt({ role: 'anon' }), 'not-a-key', ''])(
    'rejects a non-service key without revealing it',
    (key) => {
      expect(() => new SupabaseService(configure(key))).toThrow();
      if (key) {
        try {
          new SupabaseService(configure(key));
        } catch (e) {
  ...
  it.each(['sb_secret_test', jwt({ role: 'service_role' })])(
    'retains service credentials after separate concurrent logins (%s)',
    async (key) => {
      ...
      const database = requests.find((request) =>
        request.url.includes('/rest/v1/active_sessions'),
      )!;
      expect(database.headers.get('apikey')).toBe(key);
      expect(database.headers.get('Authorization')).toBe('Bearer ' + key);
      expect(database.headers.get('Authorization')).not.toBe(
        'Bearer ' + userToken,
      );
      expect(() =>
        Reflect.get(service.getAdminClient().auth, 'signInWithPassword'),
      ).toThrow();
    },
  );
});
